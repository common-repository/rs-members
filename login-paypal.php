...
define('CLIENT_ENABLED', get_option('rs_paypal_api_enabled'));
define('CLIENT_ID', get_option('rs_paypal_api_id'));
define('CLIENT_SECRET', get_option('rs_paypal_api_secret'));
define('REDIRECT_URI', rtrim(site_url(), '/') . '/');
define('SCOPE', 'openid'); // PROVIDER SPECIFIC: 'openid' is the minimum scope required to get the user's id from paypal
define('URL_AUTH', "https://www." . (get_option('rs_paypal_api_sandbox_mode') ? "sandbox." : "") . "paypal.com/webapps/auth/protocol/openidconnect/v1/authorize?");
define('URL_TOKEN', "https://api." . (get_option('rs_paypal_api_sandbox_mode') ? "sandbox." : "") . "paypal.com/v1/identity/openidconnect/tokenservice?");
define('URL_USER', "https://api." . (get_option('rs_paypal_api_sandbox_mode') ? "sandbox." : "") . "paypal.com/v1/identity/openidconnect/userinfo/?schema=openid");
...
function get_oauth_token($wpoa) {
	...
	switch (strtolower(HTTP_UTIL)) {
		case 'curl':
			...
			break;
		case 'stream-context':
			$url = $rs->rs_add_basic_auth(rtrim(URL_TOKEN, "?"), CLIENT_ID, CLIENT_SECRET); // PROVIDER SPECIFIC: Reddit/PayPal requires basic authentication with the request
			$opts = array('http' =>
				array(
					'method'  => 'POST',
